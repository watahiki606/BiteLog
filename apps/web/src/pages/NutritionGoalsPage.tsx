        ...
        <div className="max-w-md space-y-4">
          ...
          >
            ...
            <div className="text-4xl font-bold text-neon-yellow">
              {targetCalories}<span className="text-sm font-normal ml-2 text-muted-foreground">kcal / day</span>
            </div>
          </motion.div>

          {fields.map((f, i) => (
            <motion.div key={f.key} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
          ...
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            ...
            style={{ background: 'rgba(0,229,255,0.1)', border: '1px solid #00e5ff', color: '#00e5ff', boxShadow: '0 0 20px rgba(0,229,255,0.2)' }}
          >{saving ? 'SAVING...' : 'SAVE GOALS'}</motion.button>
        </div>
      </div>
    </div>
  );
