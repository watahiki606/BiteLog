...
残っている確認:
- サインインが必要な画面（ログ・フード・統計）は実データで開けていない。
  部品単位は `DesignSystemGalleryTests` の描画で確認済みだが、
  `List` の構成そのものは `ImageRenderer` が `List` を描画できないため未確認。
