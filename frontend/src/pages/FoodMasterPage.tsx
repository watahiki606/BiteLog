...
import { api, type FoodMaster } from '@/lib/api';
...
const emptyForm: Omit<FoodMaster, 'id' | 'usageCount' | 'createdBy' | 'uniqueKey'> = {
  brandName: '',
  productName: '',
  calories: 0,
  protein: 0,
...
export default function FoodMasterPage({ onToast }: Props) {
  ...
  const fetchItems = useCallback(async (q: string, off: number) => {
    setLoading(true);
    try {
      const data = await api.foodMasters.list(q, LIMIT, off);
      setItems(data.items);
      setTotal(data.total);
    } catch {
  ...
  async function handleSave() {
    try {
      if (editing) {
        await api.foodMasters.update(editing.id, form);
        onToast({ message: '更新しました', type: 'success' });
      } else {
        const id = crypto.randomUUID();
        const uniqueKey = `${form.brandName}|${form.productName}|${form.portionUnit}`;
        await api.foodMasters.create({ ...form, id, uniqueKey });
  ...
  async function handleDelete(item: FoodMaster) {
    try {
      await api.foodMasters.delete(item.id);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border-dim">
        <div>
          <h2 className="text-sm font-bold tracking-widest text-neon-cyan text-glow-cyan uppercase">
            Food Master
          </h2>
          ...
          <thead className="sticky top-0 bg-bg-surface">
            <tr>
              {['Product', 'Brand', 'kcal', 'P', 'F', 'C', 'Fiber', 'Portion', 'Used', ''].map((h) => (
                <th key={h} className="px-4 py-3 text-left text-muted-foreground tracking-widest font-normal border-b border-border-dim">
                  {h}
                </th>
              ))}
            </tr>
              ...
              items.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className="px-1.5 py-0.5 text-xs" style={{ border: '1px solid #1a1a3e', color: '#6060a0' }}>
                      {item.usageCount}
                    </span>
                  </td>
                  <td className="px-4 py-3">
        ...
        {modalOpen && (
          <Modal title={editing ? 'EDIT FOOD' : 'ADD FOOD'} onClose={() => setModalOpen(false)} onSave={handleSave}>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Product Name" colSpan={2}>
                <input type="text" value={form.productName} onChange={(e) => setField('productName', e.target.value)} className={fieldClass} style={{ fontFamily: 'inherit' }} />
              </Field>
              <Field label="Brand Name" colSpan={2}>
                <input type="text" value={form.brandName} onChange={(e) => setField('brandName', e.target.value)} className={fieldClass} style={{ fontFamily: 'inherit' }} />
              </Field>
              <Field label="Calories (kcal)">
                <input type="number" value={form.calories} onChange={(e) => setField('calories', Number(e.target.value))} className={fieldClass} style={{ fontFamily: 'inherit' }} />
              </Field>
              <Field label="Protein (g)">
                <input type="number" step="0.1" value={form.protein} onChange={(e) => setField('protein', Number(e.target.value))} className={fieldClass} style={{ fontFamily: 'inherit' }} />
              </Field>
              <Field label="Fat (g)">
                <input type="number" step="0.1" value={form.fat} onChange={(e) => setField('fat', Number(e.target.value))} className={fieldClass} style={{ fontFamily: 'inherit' }} />
              </Field>
              <Field label="Net Carbs (g)">
                <input type="number" step="0.1" value={form.netCarbs} onChange={(e) => setField('netCarbs', Number(e.target.value))} className={fieldClass} style={{ fontFamily: 'inherit' }} />
              </Field>
              <Field label="Fiber (g)">
                <input type="number" step="0.1" value={form.dietaryFiber} onChange={(e) => setField('dietaryFiber', Number(e.target.value))} className={fieldClass} style={{ fontFamily: 'inherit' }} />
              </Field>
              <Field label="Portion Size">
                <input type="number" step="0.1" value={form.portionSize} onChange={(e) => setField('portionSize', Number(e.target.value))} className={fieldClass} style={{ fontFamily: 'inherit' }} />
              </Field>
              <Field label="Portion Unit">
                <input type="text" value={form.portionUnit} onChange={(e) => setField('portionUnit', e.target.value)} className={fieldClass} style={{ fontFamily: 'inherit' }} />
              </Field>
            </div>
          </Modal>
        )}
...
const fieldClass = "w-full bg-bg-card border border-border-dim px-3 py-1.5 text-xs text-foreground focus:outline-none focus:border-neon-cyan transition-colors";
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-md bg-bg-surface border border-border-dim p-6 relative"
      ...
      >
        ...
        <div className="flex justify-end gap-3">
          ...
          <motion.button
            ...
            style={{
              background: danger ? 'rgba(255,51,102,0.1)' : 'rgba(0,229,255,0.1)',
              border: `1px solid ${danger ? '#ff3366' : '#00e5ff'}`,
              color: danger ? '#ff3366' : '#00e5ff',
              boxShadow: `0 0 10px ${danger ? 'rgba(255,51,102,0.2)' : 'rgba(0,229,255,0.2)'}`,
            }}
          >
            {saveLabel}
          </motion.button>
